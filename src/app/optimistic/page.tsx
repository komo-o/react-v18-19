...
export default function ServerActionPage() {
  ...
  const [displayCount, setDisplayCount] = useOptimistic(
    count,
    (prevCount, newCountValue: number) => {
      return prevCount + newCountValue;
    }
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
        <h2 className="text-3xl font-bold text-gray-900 text-center mb-8">楽観的更新</h2>
        ...
        <div className="mt-4 text-cente text-gray-700">送信回数: {displayCount}</div>
      </div>
    </div>
  );
}
